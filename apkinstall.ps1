...
function Msg {
    ...
    switch ("$Global:Lang`:$Key") {
        ...
        'zh_CN:processing_files' { '开始处理 .apk 文件（逐个上传并安装）……'; break }
        'zh_TW:processing_files' { '開始處理 .apk 檔案（逐一上傳並安裝）……'; break }
        'en:processing_files'    { 'Starting to process .apk files (upload & install one by one)...'; break }
        'fr:processing_files'    { 'Traitement des fichiers .apk (téléversement et installation un par un)...'; break }
        ...
        'zh_CN:install_failed' { '安装失败，错误信息如下（来自 apk）：'; break }
        'zh_TW:install_failed' { '安裝失敗，錯誤訊息如下（來自 apk）：'; break }
        'en:install_failed'    { 'Install failed, error output from apk:'; break }
        'fr:install_failed'    { "Échec de l'installation, sortie d'erreur d'apk :"; break }
        ...
        'zh_CN:force_installing' { '正在尝试强制安装（--force-broken-world）……'; break }
        'zh_TW:force_installing' { '正在嘗試強制安裝（--force-broken-world）……'; break }
        'en:force_installing'    { 'Trying force install (--force-broken-world)...'; break }
        'fr:force_installing'    { "Tentative d'installation forcée (--force-broken-world)..."; break }
...
function Test-WindowsVersion {
    ...
    switch ($Global:Lang) {
        'zh_CN' { Write-Host "检测到系统：$product $disp (内部版本 $build.$ubr)" }
        'zh_TW' { Write-Host "偵測到系統：$product $disp (內部版本 $build.$ubr)" }
        'fr'    { Write-Host "Système détecté : $product $disp (build interne $build.$ubr)" }
        default { Write-Host "Detected system: $product $disp (build $build.$ubr)" }
    }

    $ok = $false
    if ($build -ge 19044 -and $build -lt 22000) { $ok = $true }
    elseif ($build -ge 22631) { $ok = $true }
...
function Test-FileNamesSafe {
    ...
    foreach ($p in $Paths) {
        ...
        if ($name -notmatch '^[A-Za-z0-9._-]+$') { $invalid += $name }
    }

    if ($invalid.Count -gt 0) {
        Write-Host (Msg 'filename_invalid_header')
        foreach ($n in $invalid) { Write-Host "  - $n" }
    ...
    while ($true) {
        ...
        switch ($choice) {
            ...
            '4' {
                ...
                } else { Write-Host (Msg 'invalid_choice') }
            }
            default { Write-Host (Msg 'invalid_choice') }
        }
    }
...
if ([string]::IsNullOrWhiteSpace($portInput)) { $script:SshPort = 22 }
else { $script:SshPort = [int]$portInput }
